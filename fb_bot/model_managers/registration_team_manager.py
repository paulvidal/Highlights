...
from fb_highlights.models import RegistrationTeam


def get_teams_for_user(fb_id):
    teams = RegistrationTeam.objects.filter(user_id=fb_id)
...
def get_users_for_team(team_name):
    ...
    teams = RegistrationTeam.objects.filter(team_name=team)
    return [team.user.facebook_id for team in teams]


def add_team(fb_id, team_name):
    team = football_team_manager.get_football_team(team_name)
    RegistrationTeam.objects.update_or_create(user_id=fb_id, team_name=team)


def delete_team(fb_id, team_name):
    team = football_team_manager.get_football_team(team_name)
    RegistrationTeam.objects.filter(user_id=fb_id, team_name=team).delete()
